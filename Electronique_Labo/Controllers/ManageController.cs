...
namespace Electronique_Labo.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        [HttpPost]
        public ActionResult addimg(ViewModel file,HttpPostedFileBase Pimg)
        {
            if (Pimg == null)
            {
            ...
            if (find != null)
            {
                db.Profileimgs.Remove(find);
                var variable = Path.Combine(Server.MapPath("~/profileimage"), Pimg.FileName);
                Pimg.SaveAs(variable);
                var Profilefile = new profileimg();
                Profilefile.image = Pimg.FileName;
                Profilefile.ApplicationUserId = User.Identity.GetUserId();
                db.Profileimgs.Add(Profilefile);
                db.SaveChanges();
            }
        else
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ChangePassword(ViewModel model)
        {
            if (!ModelState.IsValid)
            {
                ViewBag.err = 350;
